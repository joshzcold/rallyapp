...
setListeners(BuildContext context) async {
  ...
  final FirebaseStorage storage =
      FirebaseStorage(storageBucket: "gs://game-schedule-c9299.appspot.com");
  ...
  setUserFriendListeners() async {
    ...
    database.reference().child('user/$uid/info').onChildChanged.listen((event) {
      ...
      authBloc
          .dispatch(ReplaceAuthInfo(event.snapshot.key, event.snapshot.value));
    });

    // Setting Listener on Users Friends List CHANGE
    database
        .reference()
        .child('user/$uid/friends')
        .onChildChanged
        .listen((event) {
      print(' -- CHANGE -- friend list item notify');
      notifyBloc
          .dispatch(ChangeNotify(event.snapshot.key, event.snapshot.value));
    });
    ...
    database
      ...
      if (a != null) {
        ...
        if (partyBefore.length < partyAfter.length) {
          ...
          var startTime = DateTime.fromMillisecondsSinceEpoch(
              event.snapshot.value['start']);
          var endTime =
              DateTime.fromMillisecondsSinceEpoch(event.snapshot.value['end']);

          var friendNotifyStatus = currentNotify.notify[foundFriend['id']];

          if (friendNotifyStatus['notifyJoined'] == true ||
              friendNotifyStatus['notifyJoined'] == null) {
            _showNotification(
                '${foundFriend['userName']} has joined your event! @$startTime - $endTime',
                '$eventTitle',
                "joinedFriend," + event.snapshot.key.toString() + ',$uid');
          }
        }
      }
      ...
      eventBloc.dispatch(
          ReplaceEventInfo(event.snapshot.key, event.snapshot.value, uid));
    });
    ...
    database
      ...
      eventBloc
          .dispatch(AddEvents(uid, event.snapshot.key, event.snapshot.value));
    });
    ...
    database
        .reference()
        .child('user/$uid/invites')
        .onChildAdded
        .listen((Event event) {
      var inviteUser = event.snapshot.value;

      _showNotification(
          'Friend invite from ${inviteUser['userName']}',
          'Rally ID: ${inviteUser['rallyID']}',
          "friendInvite," + event.snapshot.key.toString() + ',$uid');
    ...
    database
      ...
      database
          .reference()
          .child('user/$friendID/info')
          .once()
          .then((snapshot) async {
        print('Grabbing friend Info: ${snapshot.value}');
        if (snapshot.value == null) {
          print("Found friend without info directory: $friendID");
        } else {
          var check = snapshot.value['userPhoto'];
          if (check == null) {
            var userInfo = Map.of(snapshot.value);
            var url =
                await storage.ref().child('photos/default').getDownloadURL();
            userInfo['userPhoto'] = url;
      ...
      subscriptions[friendID][i++] = database
          .reference()
          .child('user/$friendID/info')
          .onChildChanged
          .listen((event) {
        print(' -- CHANGED -- friend info');
        friendBloc.dispatch(ReplaceFriendInfo(
            event.snapshot.key, event.snapshot.value, friendID));
      });

      // Grabbing friend event data and putting it into the notification check
      await database
          .reference()
          .child('user/$friendID/events')
          .once()
          .then((snapshot) {
        notificationCheck[friendID] = snapshot.value;
        print('Adding friend event data to notification check');
      }).catchError((error) => print(error));
      // NOTIFICATION CHECK
      ...
      subscriptions[friendID][i++] = database
        ...
        if (friendsEvents == null ||
            !friendsEvents.containsKey(event.snapshot.key)) {
          ...
          var friendNotificationCheck = notificationCheck[friendID];
          if (friendNotificationCheck != null) {
            if (!friendNotificationCheck.containsKey(event.snapshot.key)) {
              if (friendNotifyStatus['notifyEvent'] == true ||
                  friendNotifyStatus['notifyEvent'] == null) {
      ...
      database
        ...
        data.forEach((key, value) => {value.cancel()});
      });
    });
    //////////////////////////////////////////////////////////////////////////////
    //////////////////////////////////////////////////////////////////////////////
  ...
  void getRallyID() {
    ...
    database
        ...
        .child('rally/')
        .once()
        .then((snapshot) => {items = snapshot.value})
        .then((something) {
      ...
      items.containsKey(rallyID)
          ? getRallyID()
          : createInitialUserData(rallyID);
    }).catchError((error) => print(error));
  }
...
Future<void> _showNotification(title, body, payload) async {
  var androidPlatformChannelSpecifics = AndroidNotificationDetails(
      'rallyupapp',
      'rally-notification',
      'notifications from rally up. Events, Invites, Alerts',
